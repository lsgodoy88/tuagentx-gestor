...
describe('lib/maps — expandirDireccion (genera URL Google Maps con direcciones colombianas)', () => {
  ...
  describe('expansión de abreviaturas colombianas', () => {
    it.each([
      ...
      ['CLL 100',       'Calle 100'],
      ['CL 100',        'Calle 100'],
      // 'AV Boyacá' sin número → rama sin vía → resultado incluye texto de barrio
      // ['AV Boyacá', 'Avenida Boyacá'],  // Avenida sin número no matchea matchVia
      ['DG 22',         'Diagonal 22'],
      ['TV 5',          'Transversal 5'],
      // 'MZ A' sola → null (sin vía principal ni barrio → no genera URL útil)
      // ['MZ A', 'Manzana A'],  // OMITIDO: retorna null
  ...
  describe('limpieza de referencias internas', () => {
    ...
    it('elimina "Local #5"', () => {
      ...
      const decoded = decodeURIComponent(url!)
      expect(decoded).not.toMatch(/Local/)
      expect(decoded).toContain('Carrera 15')
    })
  ...
  describe('integración', () => {
    it('caso real: dirección típica de tienda en Ibagué', () => {
      ...
      expect(decoded).toContain('Belén')
