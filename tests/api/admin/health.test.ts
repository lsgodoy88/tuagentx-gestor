...
// Mock fs para que el check de build no falle ni afecte healthy
vi.mock('fs', async (importOriginal) => {
  const actual = await importOriginal<typeof import('fs')>()
  return {
    ...actual,
    readFileSync: vi.fn((p: string) => {
      if (String(p).includes('BUILD_ID')) return 'staging'
      if (String(p).includes('version.json')) return JSON.stringify({ commit: 'staging' })
      return actual.readFileSync(p)
    }),
  }
})

// Forzar DATABASE_URL para que el schema check espere 'gestor_staging'
process.env.DATABASE_URL = 'postgresql://user:pass@localhost/evolution?schema=gestor_staging'

// Helper: configura $queryRaw para SELECT 1 y SELECT current_schema() en orden
function mockQueryRawOK() {
  vi.mocked(prisma.$queryRaw)
    .mockResolvedValueOnce([{ '?column?': 1 }] as any)                    // SELECT 1
    .mockResolvedValueOnce([{ current_schema: 'gestor_staging' }] as any) // current_schema
}
