...
export default function DashboardPage() {
  ...
  return (
    ...
    {modalRecaudoRapido && (
      <div className="fixed inset-0 bg-black/90 flex items-start justify-center z-50 p-4 pt-16"
        onClick={e => { if (e.target === e.currentTarget) { setModalRecaudoRapido(false); setRrCliente(null); setRrSinDeuda(false) } }}>
        <div className="bg-zinc-950 border border-blue-500/25 rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-blue-500/20">
            ...
            {!rrCliente ? (
              <>
                <input
                  ...
                  className="w-full bg-black border border-blue-500/25 rounded-xl px-4 py-2.5 text-white text-sm outline-none focus:border-blue-500"
                  autoFocus
                />
                {rrLoadingCartera && (
    ...
    {recaudandoCartera && (() => {
      ...
      return (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4">
          <div className="bg-zinc-950 border border-blue-500/25 rounded-2xl w-full max-w-md space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 space-y-4 pb-6">
              <div className="bg-black border border-blue-500/25 rounded-xl px-4 py-3">
                ...
                <>
                  {detalleData.DetalleCartera?.filter((d: any) => d.estado !== 'pagada').length > 0 && (
                    <div className="space-y-1.5">
                      ...
                      {detalleData.DetalleCartera.filter((d: any) => d.estado !== 'pagada').map((d: any) => {
                        ...
                        return (
                          <label key={d.id} className={`flex items-center gap-3 bg-black border rounded-xl px-4 py-2.5 cursor-pointer transition-colors ${seleccionada ? 'border-emerald-500/50' : 'border-blue-500/25 hover:border-blue-500/40'}`}>
                  ...
                  <div className="space-y-3">
                    {lineasPago.map((linea, idx) => (
                      <div key={linea.id} className="bg-black border border-blue-500/25 rounded-xl p-4 space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {(['efectivo', 'transferencia'] as const).map(met => (
                            <button key={met} onClick={() => setLineasPago(prev => prev.map(l => l.id === linea.id ? { ...l, metodoPago: met, voucherKey: null, voucherDatosIA: null, cargandoVoucher: false } : l))}
                              className={`py-2 rounded-xl text-xs font-semibold border transition-colors ${linea.metodoPago === met ? 'bg-blue-500/10 border-blue-500/40 text-white' : 'bg-black border-blue-500/25 text-zinc-400 hover:text-white'}`}>
                          ...
                          <div className="flex gap-3">
                            <div className="flex-[7]">
                              ...
                              <InputMoneda value={linea.monto}
                                ...
                                className="w-full bg-black border border-blue-500/25 rounded-xl pr-4 py-2.5 text-white text-sm outline-none focus:border-blue-500" />
                            </div>
                          ...
                          <div className="space-y-3">
                            ...
                            {!linea.voucherKey && !linea.cargandoVoucher && (
                              <button onClick={() => fileInputRefs.current.get(linea.id)?.click()}
                                className="w-full bg-black border border-dashed border-blue-500/25 rounded-xl py-2.5 text-zinc-400 text-sm hover:text-white hover:border-blue-500 transition-colors">
                                📎 Adjuntar comprobante
                              </button>
                            )}
                            {linea.cargandoVoucher && (
                              <div className="bg-black border border-blue-500/25 rounded-xl px-4 py-3 text-zinc-400 text-sm text-center animate-pulse">
                                Analizando comprobante con IA...
                              </div>
                            )}
                            {linea.voucherDatosIA && !linea.cargandoVoucher && (
                              <div className="bg-black border border-blue-500/30 rounded-xl px-4 py-3 space-y-2.5">
                                <div className="flex items-center justify-between">
                              ...
                              <div className="flex gap-3">
                                <div className="flex-[7]">
                                  ...
                                  <InputMoneda value={linea.monto} readOnly
                                    className="w-full bg-black/50 border border-blue-500/20 rounded-xl pr-4 py-2.5 text-zinc-300 text-sm outline-none cursor-not-allowed" onChange={() => {}} />
                                </div>
                                <div className="flex-[3]">
                                  <label className="text-zinc-400 text-xs font-semibold block mb-1.5">Descuento</label>
                                  <InputMoneda value={linea.descuento} placeholder="0" prefix=""
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="bg-black border border-blue-500/25 rounded-xl px-4 py-3 space-y-1.5">
