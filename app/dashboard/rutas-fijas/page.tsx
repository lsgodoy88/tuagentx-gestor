...
<div className="flex gap-1 tab-pills rounded-xl p-1">
        <button onClick={() => setTab('rutas')} className={`flex-1 py-3 text-sm font-semibold transition-colors text-center ${tab === 'rutas' ? 'tab-active' : 'text-white hover:text-white'}`}>Rutas fijas</button>
        <button onClick={() => setTab('historial')} className={`flex-1 py-3 text-sm font-semibold transition-colors text-center ${tab === 'historial' ? 'tab-active' : 'text-white hover:text-white'}`}>Historial</button>
        {syncVentas && (
          <button onClick={ejecutarSyncVentas} disabled={sincronizandoVentas}
            className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-semibold text-white/70 hover:text-white transition-colors disabled:opacity-40 ${sincronizandoVentas ? 'btn-shimmer' : ''}`}>
