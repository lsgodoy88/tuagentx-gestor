...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (isEmpleado) {
      const empleadoFetches: Promise<any>[] = [
      ...
      if (user.role === 'vendedor') {
        ...
        empleadoFetches.push(fetch('/api/vendedor/stats').then(r => r.json()).catch(() => null))
      }
      Promise.all(empleadoFetches).then(([r, t, me, stats]) => {
        setRuta(r)
        ...
        setTurno(t)
        if (!t) setTurnoExpandido(true)
        setPuedeCapturarGps(me?.puedeCapturarGps === true)
        if (stats) { setStatsVendedor(stats); setLoadingStats(false) }
  ...
  return (
          ...
          {monitor.length > 0 && (
            <div className="space-y-4">
              {['vendedor', 'impulsadora', 'entregas'].map(rol => {
                ...
                return (
                  <div key={rol} className="rounded-2xl overflow-hidden" style={{background:"rgba(8,8,28,0.82)",border:"1px solid rgba(59,130,246,0.25)"}}>
                    <div className="px-4 py-3 border-b border-zinc-800">
      ...
      {isEmpleado && (
        <div className="space-y-4">
          {turno?.pausado ? (
              ...
              {turnoExpandido && (
                <div className="border-t border-amber-500/20 px-4 pb-4 pt-3 space-y-3">
                  <div className="grid grid-cols-2 gap-2">
                    <div style={{background:"rgba(15,15,22,0.60)", border:"1px solid rgba(59,130,246,0.20)", borderRadius:8, padding:8}}><p className="text-zinc-500 text-xs">Inicio pausa</p><p className="text-sm font-bold text-white">{turno.pausaInicio ? new Date(turno.pausaInicio).toLocaleTimeString("es-CO",{hour:"2-digit",minute:"2-digit"}) : "--"}</p></div>
                    <div style={{background:"rgba(15,15,22,0.60)", border:"1px solid rgba(59,130,246,0.20)", borderRadius:8, padding:8}}><p className="text-zinc-500 text-xs">Reanuda a las</p><p className="text-emerald-400 text-sm font-bold">{new Date(new Date(turno.pausaInicio).getTime() + turno.pausaDuracionMin*60000).toLocaleTimeString("es-CO",{hour:"2-digit",minute:"2-digit"})}</p></div>
                  </div>
            ...
            <div style={{background:"rgba(8,8,28,0.82)", border:"1px solid rgba(59,130,246,0.30)", borderRadius:16, overflow:"hidden"}}>
              ...
              {turnoExpandido && (
                <div className="border-t border-emerald-500/20 px-4 pb-4 pt-3 space-y-3">
                  <div className="grid grid-cols-2 gap-2">
                    <div style={{background:"rgba(15,15,22,0.60)", border:"1px solid rgba(59,130,246,0.20)", borderRadius:8, padding:8}}><p className="text-zinc-500 text-xs">Hora inicio</p><p className="text-sm font-bold text-white">{new Date(turno.inicio).toLocaleTimeString("es-CO",{hour:"2-digit",minute:"2-digit"})}</p></div>
                    <div style={{background:"rgba(15,15,22,0.60)", border:"1px solid rgba(59,130,246,0.20)", borderRadius:8, padding:8}}><p className="text-zinc-500 text-xs">Contador</p><p className="text-emerald-400 font-mono font-bold">{tiempoTurno}</p></div>
                  </div>
            ...
            <div style={{background:"rgba(8,8,28,0.82)", border:"1px solid rgba(59,130,246,0.25)", borderRadius:16, overflow:"hidden"}}>
              {/* Pill encogida */}
              <button onClick={() => setTurnoExpandido(e => !e)}
                className="w-full flex items-center gap-3 px-4 py-3 text-left">
          ...
          {user?.role === 'vendedor' && (
            <div className="space-y-4">
              {/* Hoy — siempre visible */}
              {statsVendedor && (
                <div className="rounded-2xl p-4 fade-up" style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.30)",boxShadow:"0 4px 24px rgba(0,0,0,0.25),inset 0 1px 0 rgba(255,255,255,0.25)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)"}}>
                  <p className="text-white font-bold mb-3">Hoy</p>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-xl p-3" style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.30)",boxShadow:"0 4px 24px rgba(0,0,0,0.25),inset 0 1px 0 rgba(255,255,255,0.25)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)"}}>
    ...
    {modalRecaudoRapido && (
      <div className="fixed inset-0 bg-black/95 flex items-start justify-center z-50 p-4 pt-16"
                ...
                <>
                  ...
                  <div className="space-y-3">
                    {lineasPago.map((linea, idx) => (
                      <div key={linea.id} className="rounded-xl p-4 space-y-3" style={{background:"rgba(30,32,48,0.98)",border:"1px solid rgba(59,130,246,0.20)"}}>
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="rounded-xl px-4 py-3 space-y-1.5" style={{background:"rgba(30,32,48,0.98)",border:"1px solid rgba(59,130,246,0.20)"}}>
