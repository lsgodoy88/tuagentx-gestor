...
const PAGE_SIZE_CARTERA = 15

const ESTADO_COLOR_TABLE: Record<string, string> = {
  critica: '#dc2626', mora: '#f43f5e', vencida: '#f97316',
  proxima: '#f59e0b', pendiente: '#eab308', vigente: '#3b82f6',
  abonada: '#3b82f6', pagada: '#22c55e',
}
const ESTADO_LABEL_TABLE: Record<string, string> = {
...
function estadoPpalTable(porEstado: any): string {
...
function getCarteraColumns(ctx: {
  onRecaudar: (c: any) => void
  onWhatsApp: (c: any) => void
  esAdmin: boolean
}): ColDef<any>[] {
  return [
    {
      ...
      render: (c: any) => (
        <span style={{ fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', textAlign: 'left', display: 'block' }}>
          {c.cliente?.nombre || c.nombre || '—'}
        </span>
      ),
    },
    {
      key: 'nit', label: 'NIT', width: 100, minWidth: 70,
      render: (c: any) => (
    ...
    {
      key: 'saldo', label: 'Saldo', width: 130, minWidth: 90,
      render: (c: any) => (
        <span style={{ color: '#fde68a', fontWeight: 700 }}>
          {'$' + Math.round(Number(c.saldoPendiente)).toLocaleString('es-CO')}
        </span>
      ),
    },
    {
      key: 'estado', label: 'Estado', width: 120, minWidth: 80,
      render: (c: any) => {
        const est = estadoPpalTable(c.porEstado)
        const color = ESTADO_COLOR_TABLE[est] || '#6366f1'
        return (
          <span style={{ fontSize: 11, fontWeight: 700, color, border: `1px solid ${color}40`, borderRadius: 6, padding: '2px 8px', whiteSpace: 'nowrap' }}>
            {ESTADO_LABEL_TABLE[est] || est}
          </span>
        )
      },
    },
    {
      ...
      render: (c: any) => <span>{c.empleado?.nombre?.split(' ')[0] || '—'}</span>,
    },
    {
      key: 'acciones', label: 'Acciones', width: 120, minWidth: 90,
      render: (c: any) => {
        const tel = (c.cliente?.celular || c.cliente?.telefono || c.telefono || c.celular || '').replace(/\D/g, '')
        return (
          <div style={{ display: 'flex', gap: 8, justifyContent: 'center', alignItems: 'center' }}>
            {Number(c.saldoPendiente) > 0 && (
              <button onClick={e => { e.stopPropagation(); ctx.onRecaudar(c) }}
                style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 16, padding: 0, lineHeight: 1 }} title="Recaudar">💰</button>
            )}
            {tel && (
              <a href={`https://wa.me/57${tel}`} target="_blank" rel="noreferrer"
...
export default function CarteraPage() {
  ...
  const [totalSaldoReal,    setTotalSaldoReal]    = useState(0)
  const [totalCartReal,     setTotalCartReal]     = useState(0)
  const [totalClientesReal, setTotalClientesReal] = useState(0)
  ...
  async function cargarDatos(q = '') {
    ...
    if (!q) {
      ...
      if (cached) {
        ...
        setPagos(r2.pagos || [])
        setMetas(r3.metas || [])
        if (r1.totalSaldoPendiente) setTotalSaldoReal(r1.totalSaldoPendiente)
        if (r1.totalSaldoTotal)     setTotalCartReal(r1.totalSaldoTotal)
        if (r1.total)               setTotalClientesReal(r1.total)
        ...
        Promise.all([
          ...
          if (nr1.carteras) {
            ...
            setPagos(nr2.pagos || [])
            setMetas(nr3.metas || [])
            if (nr1.totalSaldoPendiente) setTotalSaldoReal(nr1.totalSaldoPendiente)
            if (nr1.totalSaldoTotal)     setTotalCartReal(nr1.totalSaldoTotal)
            if (nr1.total)               setTotalClientesReal(nr1.total)
            setOffline(false)
  ...
  async function sincronizar() {
    ...
    clearCache('cartera')  // forzar datos frescos tras sync
  ...
  const pagedFiltradas = filtradas.slice(page * PAGE_SIZE_CARTERA, (page + 1) * PAGE_SIZE_CARTERA)
  const totalPagesCartera = Math.max(1, Math.ceil(filtradas.length / PAGE_SIZE_CARTERA))
  ...
  return (
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {tab === 'cartera' && (() => {
        ...
        const totalCartera = totalCartReal  || carteras.reduce((s: number, c: any) => s + (c.DetalleCartera || []).reduce((a: number, d: any) => a + Number(d.valorFactura ?? d.valor ?? 0), 0), 0)
        const totalPend     = totalSaldoReal || carteras.reduce((s: number, c: any) => s + Number(c.saldoPendiente), 0)
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className={`rounded-2xl p-4 hover-lift fade-up stagger-1 ${loadingBusqueda ? 'loading-border' : ''}`} style={{ background:"rgba(8,8,28,0.82)", border:"1px solid rgba(59,130,246,0.25)", borderRadius:16, padding:16 }}>
                ...
                <p className="text-zinc-600 text-xs mt-1"><CountUp end={totalClientesReal || carteras.length} /> clientes</p>
              </div>
              <div className={`rounded-2xl p-4 hover-lift fade-up stagger-2 ${loadingBusqueda ? 'loading-border-red' : ''}`} style={{ background:"rgba(8,8,28,0.82)", border:"1px solid rgba(59,130,246,0.25)", borderRadius:16, padding:16 }}>
              ...
              <div className={`rounded-2xl p-4 hover-lift fade-up stagger-3 ${loadingBusqueda ? 'loading-border-emerald' : ''}`} style={{ background:"rgba(8,8,28,0.82)", border:"1px solid rgba(59,130,246,0.25)", borderRadius:16, padding:16 }}>
              ...
              <div className={`rounded-2xl p-4 hover-lift fade-up stagger-4 ${loadingBusqueda ? 'loading-border-amber' : ''}`} style={{ background:"rgba(8,8,28,0.82)", border:"1px solid rgba(59,130,246,0.25)", borderRadius:16, padding:16 }}>
            ...
            {/* Estado + Vendedor + Meta — grid 3 cols en desktop */}
      ...
      {tab === 'clientes' && (
        <div className="space-y-3">
          <input value={buscar} onChange={e => onBuscarChange(e.target.value)}
            ...
            style={{ background:"rgba(8,8,28,0.82)", border:"1px solid rgba(59,130,246,0.30)", borderRadius:12, padding:"10px 16px", color:"white", fontSize:14, outline:"none", width:"100%" }} />
          {isDesktop ? (
            <div style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)' }}>
              <DataTable
                columns={getCarteraColumns({ onRecaudar: abrirRecaudar, onWhatsApp: abrirWhatsApp, esAdmin })}
                rows={pagedFiltradas}
                rowKey={(c: any) => c.id}
                loading={loading}
                storageKey="cartera-clientes"
              />
            </div>
          ) : (
            <div>
              ...
              {filtradas.length === 0 && (
                <div style={{ background:"rgba(8,8,28,0.82)", border:"1px solid rgba(59,130,246,0.25)", borderRadius:16, padding:40, textAlign:"center" }}>
            ...
            filtradas.length > 0 && (
              <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,paddingTop:8}}>
                ...
                <span style={{fontSize:12,color:'rgba(255,255,255,0.6)',minWidth:90,textAlign:'center'}}>
                  Pág {page + 1} / {totalPagesCartera}{hayMas ? '+' : ''}
                </span>
                <button
                  onClick={async () => {
                    const nextPage = page + 1
                    if (nextPage >= totalPagesCartera && hayMas) await cargarMas()
                    setPage(nextPage)
                  }}
                  disabled={(page >= totalPagesCartera - 1 && !hayMas) || cargandoMas}
                  style={{background:'rgba(15,15,22,0.60)',border:'1px solid rgba(59,130,246,0.40)',borderRadius:'0.75rem',padding:'6px 16px',fontSize:12,fontWeight:700,color:(page>=totalPagesCartera-1&&!hayMas)?'rgba(255,255,255,0.25)':'white',cursor:(page>=totalPagesCartera-1&&!hayMas)?'not-allowed':'pointer'}}>
                  {cargandoMas ? '...' : 'Siguiente →'}
                </button>
                <span style={{fontSize:11,color:'rgba(255,255,255,0.3)',marginLeft:4}}>{filtradas.length} registros</span>
              </div>
            )
          ) : (
            hayMas && (
      ...
      {modalImportar && esAdmin && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95" onClick={e => { if (e.target === e.currentTarget) setModalImportar(false) }}>
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="rounded-2xl p-4 text-sm text-zinc-400 space-y-1" style={{background:"rgba(15,15,22,0.60)",border:"1px solid rgba(59,130,246,0.20)"}}>
              ...
              {preview.length > 0 && (
                <div style={{ background:"rgba(8,8,28,0.82)", border:"1px solid rgba(59,130,246,0.25)", borderRadius:16, padding:16 }} className="space-y-3">
              ...
              <div key={p.id} className="bg-zinc-900 border border-zinc-800 rounded-2xl px-4 py-3 flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-zinc-500 text-xs">
                    {new Date(p.createdAt).toLocaleDateString('es-CO', {day:'2-digit',month:'2-digit',year:'2-digit',timeZone:'America/Bogota'})} · {p.metodoPago} · {p.empleado?.nombre}
                  </p>
        ...
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-50 p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 space-y-4 pb-6">
              <div className="bg-zinc-800 rounded-xl px-4 py-3" style={{border:"1px solid rgba(59,130,246,0.40)"}}>
                ...
                <>
                  ...
                  {detalleData.DetalleCartera?.filter((d: any) => d.estado !== 'pagada').length > 0 && (
                    <div className="space-y-1.5">
                      ...
                      {detalleData.DetalleCartera.filter((d: any) => d.estado !== 'pagada').map((d: any) => {
                        ...
                        return (
                          <label key={d.id} style={{background:"#27272a",border:`2px solid ${seleccionada?"rgba(59,130,246,0.90)":"rgba(59,130,246,0.50)"}`,borderRadius:12,padding:"10px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:12}}>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {d.fechaVencimiento && <p className="text-zinc-500 text-xs">Vence: {new Date(d.fechaVencimiento).toLocaleDateString('es-CO', {day:'2-digit',month:'2-digit',year:'2-digit',timeZone:'America/Bogota'})}</p>}
                            </div>
                            <div className="text-right flex-shrink-0">
                  ...
                  <div className="space-y-3">
                    {lineasPago.map((linea, idx) => (
                      <div key={linea.id} className="bg-zinc-800 rounded-xl p-4 space-y-3" style={{border:"1px solid rgba(59,130,246,0.40)"}}>
                          ...
                          {(['efectivo', 'transferencia'] as const).map(met => (
                            <button key={met} onClick={() => setLineasPago(prev => prev.map(l => l.id === linea.id ? { ...l, metodoPago: met, voucherKey: null, voucherDatosIA: null, cargandoVoucher: false } : l))}
                              className={`py-2 rounded-xl text-xs font-semibold border transition-colors ${
                                linea.metodoPago === met ? '!border-blue-500/60 text-white' : 'border-zinc-700 text-zinc-400 hover:text-white'
                              }`}>
                          ...
                          <div className="flex gap-3">
                            <div className="flex-[6]">
                              ...
                              <InputMoneda value={linea.monto}
                                ...
                                className="w-full  rounded-xl pr-4 py-2.5 text-sm" style={{background:"rgba(15,15,22,0.60)",border:"1px solid rgba(59,130,246,0.20)",color:"white"}} />
                            </div>
                            <div className="flex-[4]">
                              ...
                              <InputMoneda value={linea.descuento} placeholder="0" prefix=""
                                ...
                                className="w-full  rounded-xl px-4 py-2.5 text-sm" style={{background:"rgba(15,15,22,0.60)",border:"1px solid rgba(59,130,246,0.20)",color:"white"}} />
                            </div>
                          </div>
                        )}
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="bg-zinc-800 rounded-xl" style={{border:"1px solid rgba(59,130,246,0.40)"}}>
                  ...
                  <div>
                    ...
                    <input value={notasPago} onChange={e => setNotasPago(e.target.value)}
                      ...
                      className="bg-zinc-800 rounded-xl" style={{border:"1px solid rgba(59,130,246,0.40)"}} />
                  </div>
    ...
    {modalSync && syncInfo?.tieneIntegracion && (
      <div className="fixed inset-0 bg-black/95 z-50 flex items-start justify-center p-4 pt-20" onClick={() => setModalSync(false)}>
