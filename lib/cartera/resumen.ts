...
export async function getResumenCartera(params: {
  empresaId: string
  empleadoIdForzado: string | null
}) {
  const { empresaId, empleadoIdForzado } = params
  const ahora = nowBogota()
  const mesClave = `${ahora.getUTCFullYear()}-${ahora.getUTCMonth()}`
  ...
  return withCache(cacheKey, 300, async () => {
    ...
    let totalCartera = 0, totalPendiente = 0, clientes = 0

    if (integracion) {
      if (empleadoIdForzado) {
        const emp = await (prisma as any).empleado.findUnique({
          where: { id: empleadoIdForzado }, select: { apiId: true },
        })
    ...
    const whereMes    = { ...base, createdAt: { gte: inicioMes }, ...(empleadoIdForzado ? { empleadoId: empleadoIdForzado } : {}) }
    const whereMesAnt = { ...base, createdAt: { gte: inicioMesAnt, lt: inicioMes }, ...(empleadoIdForzado ? { empleadoId: empleadoIdForzado } : {}) }

    const [aggMes, aggAnt] = await Promise.all([
      (prisma as any).pagoCartera.aggregate({ where: whereMes, _sum: { monto: true, descuento: true }, _count: { id: true } }),
      (prisma as any).pagoCartera.aggregate({ where: whereMesAnt, _sum: { monto: true, descuento: true } }),
    ])
