...
export async function backfillFechaFactura(params: {
  empresaId?: string
  limite?: number
}) {
  const { empresaId: empFiltro, limite = 50 } = params
  const limiteReal = Math.min(limite, 200)
  ...
  if (ordenesBug.length === 0) {
    return { ok: true, mensaje: 'Sin ordenes para corregir', corregidas: 0 }
  }
